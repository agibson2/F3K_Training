...
vbase.horizontaldividery = 91 --110 --115 -- 76
...
function vbase.drawDashboard( widget, task )
	...
	if widget.sensor_battery ~= nil then
		...
		if batteryvalue ~= nil then
			lcd.drawNumber( f3kTextOffset + 4 + text_w*6 -3, text_h*3, batteryvalue, UNIT_VOLT, 2, RIGHT)
		end
	end
	if widget.sensor_rssi ~= nil then
		...
		if rssivalue ~= nil then
			lcd.drawNumber( f3kTextOffset + 4 + text_w*6, text_h*5, rssivalue, UNIT_DB, 0, RIGHT )
		end
	end
	...
	if task ~= nil and task.name == "Free Flight" and widget.launch_height_enabled then
		if widget.sensor_vspeed == nil or widget.sensor_altitude == nil or task.launchheight == nil then
			...
			if altitudevalue ~= nil then
				lcd.drawNumber( f3kTextOffset + 4 + text_w*6, text_h*7, altitudeval, altitudevalue, 0, RIGHT )
			end
		end
	end
end
...
function vbase.drawPrepWorkTime( widget, task )
	...
	local text_w, text_h = lcd.getTextSize("0")
	local prefixString
	-- We don't want to show Prep time if we start work time on launch (prep time is disabled for that setting)
