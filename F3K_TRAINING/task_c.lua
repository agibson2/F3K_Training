...
function taskC.flyingState(widget)
	if not taskC.endOfWindow() and not taskC.earlyReset(widget) then
...
-- public interface
function taskC.init()
	if (DebugFunctionCalls) then print("FTRAIN: taskC.init()") end
	taskC.WINDOW_TIME = 30 * 60
	taskC.commonInit( 'AULD', taskC.COUNT, 'taskc' )
end
